...
} from '../decorators/query-property.decorator';
import { 
  generateSwaggerRecordDtoForEntity 
} from '../decorators/serialize-property.decorator';
import { 
  generateSwaggerCreateUpdateDtoForEntity 
} from '../decorators/create-update-property.decorator';
import { 
...
export class BaseControllerModule {
  static forEntity<T extends BaseEntity>(config: BaseControllerConfig<T>): DynamicModule {
    ...
    class DynamicBaseController extends BaseController<T> {
      ...
      @Get()
      @ApiQuery({ name: 'query', title: 'Where criteria', required: false, type: () => ListQueryDto })
      ...
      @Get('count')
      @ApiQuery({ name: 'query', title: 'Where criteria', required: false, type: () => CountQueryDto })
      async count(
      ...
      @ApiOkResponse({ type: RecordDto })
      async create(
        @Body() entity: CreateRequestDto = new CreateDto()
      ): Promise<T> {
      ...
      providers: [
        {
          ...
          inject: [
            getBaseServiceInjectToken(entity),
          ],
        }
      ],
